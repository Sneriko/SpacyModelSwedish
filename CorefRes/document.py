...
NO_COREF_LIST = ["jag", "mig", "min", "du", "din"]
...
def extract_mentions_spans(doc, blacklist, debug=False):
    ...
    for spans in parallel_process(
        [{"doc": doc, "span": sentence, "blacklist": blacklist} for sentence in doc.sents],
        _extract_from_sent,
        use_kwargs=True,
        front_num=0,
    ):
...
def _extract_from_sent(doc, span, blacklist=True, debug=False):
    ...
    """
    keep_tags = re.compile(r"^NN|^PM|^HP|^PN|^DT|^HD|^HS^PS|^KN|^PP")
    leave_dep = ["det", "compound", "appos", "expl"]
    keep_dep = ["nsubj", "obj", "iobj"]
    ...
    if any(tok.tag_ == 'PP' for tok in span):
        prepos = list(tok.i for tok in span if tok.tag_ == 'PP')
        leftedge = -1
        for prep in prepos:
            if any(tok.dep_ == 'nsubj' and tok.i < prep for tok in span):
                leftedgeList = list(tok.left_edge.i for tok in span if tok.i < prep and tok.dep_ == 'nsubj')
                leftedge = max(leftedgeList)
                break
            else:
                continue
        rightEdgeList = list(tok.i for tok in span if tok.i == doc[max(prepos)].head.i)
        rightEdge = max(rightEdgeList)

        if leftedge != -1:
            prepSpan = doc[leftedge:rightEdge + 1]
            mentions_spans.append(prepSpan)

        #nmod

        for tok in span:
            if tok.dep_ == 'nsubj' and tok.right_edge.dep_ == 'nmod':
                left = list(c.left_edge.i for c in doc if c.head.i == tok.i)
                right = [tok.i]
                start, end = cleanup_endings(left, right, tok)
                if start == end:
                    continue
                sp = doc[start:end]
                mentions_spans.append(sp)
    ...
    for token in span:
        ...
        # pronoun
        if re.match(r"^HP|^PN", token.tag_):
            if debug:
                print("PRP")
            endIdx = token.i + 1
        ...
        #Deal with the cop dependency
        cop = False
        if token.dep_ == "ROOT":
            left = list(c.i for c in doc if c.head.i == token.i and (c.dep_ == "det" or c.dep_ == "amod"))
